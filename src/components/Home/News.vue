...
export default {
  ...
  mounted() {
    this.content_height =
      "calc(100% - " + (this.$refs.Title.clientHeight + 20) + "px)";
    this.$refs.Content.style.setProperty("height", this.content_height);
    if (this.isPC) {
      ...
      for (
        let idx = 0;
        idx < this.$refs.NewsList.getElementsByTagName("b").length;
        idx++
      ) {
...
<template>
  <div class="News">
    ...
    <div class="content text-gray-700" ref="Content">
      <div
        style="
          overflow-y: auto;
          display: flex;
      ...
      >
        <ul ref="NewsList">
          <li v-for="(newsItem, key) in content" style="margin-bottom: 5px">
            <Firework
              v-if="newsItem.search('Congratulation') > 0 && isPC"
